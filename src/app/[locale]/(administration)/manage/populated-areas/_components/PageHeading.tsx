...
const PageHeading = ({ setCountryId, regions }: PageHeadingProps) => {
  ...
  const selectedValue = useMemo(() => {
    const country = countries?.find(
      (country: Country) => country.id === selectedKeys,
    );
  ...
  useEffect(() => {
    if (countries) {
      setCurrentCountry(countries[0]?.name);
    }
  }, [countries]);
  ...
  const areaCountReducer = useCallback(
    (status: string) => {
      return regions
        ?.map(
          (region) =>
            region.populatedAreas.filter((area: any) => area.status === status)
              .length,
        )
        .reduce((a, b) => a + b, 0);
    },
            ...
            <Dropdown>
              ...
              >
                {countries?.map((country: Country) => (
                  <DropdownItem key={country.id} className="fira-go">
                    {country.name}
                  </DropdownItem>
                ))}
              </DropdownMenu>
