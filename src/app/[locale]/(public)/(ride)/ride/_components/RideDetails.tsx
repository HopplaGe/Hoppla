...
import { BadgeCheck, Check, Facebook, Twitter } from "lucide-react";
...
const RideDetails = ({ ride, searchParams }: any) => {
  ...
  const arrivalTime = moment(ride.startTime, "HH:mm")
    .add(secondsToHours(ride.duration!), "seconds")
    .format("HH:mm");

  const { distance: fromDistance } = useDirections(
    searchParams.from,
    ride.from,
  );
  const { distance: toDistance } = useDirections(searchParams.to, ride.to);
  const { directionResponse, startLatLng, endLatLng } = useDirections(
    ride.from,
    ride.to,
  ...
  return (
    <>
      ...
      <section className="flex flex-col gap-4 w-full lg:w-1/2">
        ...
        >
          <div className="flex flex-col gap-2 fira-go text-md font-bold">
            {ride?.driver.name}
          </div>
        ...
        {ride?.trip?.passangers.length > 0 && (
          <div className="flex flex-row justify-between items-center p-4 fira-go hover:bg-default-100 rounded-xl hoppla-animation">
            <div className="flex flex-col gap-2 fira-go text-sm font-bold">
              {t("passengers")}
            </div>
            <div className="flex flex-col gap-2 font-bold text-xl">
              <AvatarGroup>
                {ride?.trip?.passangers &&
                  ride?.trip?.passangers?.map(
                    (passanger: any, index: number) => (
                      <Avatar
                        key={index}
                        radius="md"
                        size="sm"
      ...
      <section className="flex flex-col gap-4 w-full md:w-1/2 px-4">
        {ride?.rideRules &&
          ride.rideRules.map((rule: any, index: number) => (
            <div
              key={index}
        ...
        <div className="flex flex-row justify-between items-center border-t-4 p-4 fira-go border-default-100">
          {user?.id !== ride?.driver.id ? (
            <Button
              ...
              isDisabled={
                ride?.trip?.passangers?.length === ride?.trip?.seats ||
                ride?.trip?.passangers?.find(
                  (passanger: any) => passanger.id === user?.id,
                )
              }
            >
              დაჯავშნა
