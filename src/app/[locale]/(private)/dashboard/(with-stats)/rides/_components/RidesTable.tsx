...
export default function RidesTable({ rides }: { rides: Ride[] }) {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredRides = filteredRides.filter(
        (ride) =>
          ride.from.toLowerCase().includes(filterValue.toLowerCase()) ||
  ...
  const renderCell = React.useCallback((ride: Ride, columnKey: React.Key) => {
    ...
    switch (columnKey) {
      ...
      case "passangers":
        return (
          <AvatarGroup max={4} className="flex gap-1" title={t("passangers")}>
            {ride.trip?.passangers.map((p) => (
              <Avatar key={p.id} src={p.image!} radius={"md"} size={"sm"} />
            ))}
          </AvatarGroup>
        );
      case "price":
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between gap-3 items-end">
          ...
          <div className="flex gap-3">
            ...
            <Button color="primary" endContent={<Printer size={16} />}>
              ინვოისი
            </Button>
          </div>
        </div>
