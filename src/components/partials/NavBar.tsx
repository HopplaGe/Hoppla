      ...
      >
        <div className="w-full flex justify-between items-center">
          <NavbarContent className="sm:flex gap-4 ml-2 items-center !basis-full">
            ...
            <div className="hidden flex-initial w-full min-h-6 lg:flex ml-2 fira-go">
              {navItems.map((item, index) => (
                <NavbarItem key={index} className="h-full">
                  <Link
                    href={item.href}
                    className={cn(
                      "text-secondary lg:py-5 px-4 hover:bg-default-50 border-transparent text-sm",
                      pathName === "/" + locale + item.href &&
                        "bg-default-50 text-primary border-primary font-bold",
